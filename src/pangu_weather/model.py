...
def LoadConstantMask(batch_size):
    ...
    # Soil type is categorical tensor, and is transformed to scale [0,1]:
    unique_values = np.unique(soil_type)
    def soil_type_scaler(value):
        scale_dict = {val: i/(len(unique_values)-1) for i, val in enumerate(unique_values)}
        return scale_dict[value]
    soil_type = np.vectorize(soil_type_scaler)(soil_type)
    soil_type = torch.from_numpy(soil_type).permute(1,0).view(1,1,1440,721).to(torch.float16)
...
class PatchRecovery(nn.Module):
    ...
    def forward(self, x):
        ...
        output_surface = self.tconv_surface(x[:, :, -1, :, :])
...
class EarthSpecificBlock(nn.Module):
    ...
    def forward(self, x, roll):
        ...
        # Shifting windows:
        if roll:
            # Roll x for half of the window for 3 dimensions (Z, H, W): #TODO: check that the direction of roll matches the mask
        ...
        # Transpose into (B, nWh, nWz, nWw, self.window_size[1], self.window_size[0], self.window_size[2], C) for
        # appropriate broadcasting addition of earth specific bias in attention layer:
        x = x.permute(0,3,1,5,4,2,6,7).contiguous().view(B*nW, T, C)
        ...
        x = x.view(B, nWh, nWz, nWw, self.window_size[1], self.window_size[0], self.window_size[2], C)
        x = x.permute(0,2,5,1,4,3,6,7).contiguous().view(B, Z, H, W, C)
...
class EarthAttention3D(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        nW = nWz*nWh*nWw
        n_submatrices = nWz*nWw
        assert B_%n_submatrices == 0, f"n_submatrices is not a multiple of B*nW!"
        ...
        # Transpose and reshape positional_bias to match the shape of attention matrix:  
        # (T**2, n_submatrices, n_heads) -> (1, n_submatrices, n_heads, T, T)
        positional_bias = positional_bias.permute(1, 2, 0).view(n_submatrices, self.n_heads, T, T).unsqueeze(0)
        ...
        """
        # Reshape attention to be compatible with broadcasting addition of the bias:
        # (nW*B, n_heads, T, T) -> (B*nW/n_submatrices, n_submatrices, n_heads, T, T)
        attention = attention.view(B_//n_submatrices, n_submatrices, self.n_heads, T, T)

        # Add the Earth-specific bias to the attention matrix:
        #   (B*nW/n_submatrices, n_submatrices, n_heads, T, T) + (1, n_submatrices, n_heads, T, T)
        attention = attention + positional_bias
        del positional_bias
        attention = attention.view(B_, self.n_heads, T, T).view(B_//nW, nW, self.n_heads, T, T)
